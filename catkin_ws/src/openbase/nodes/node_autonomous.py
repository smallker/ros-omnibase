...
class NodeAutonomous:
    ...
    def heading_mode(self, pose:Pose2D):
        if self.pose_control_started:
            # self.ang_pid.sp = - self.sp_heading
            self.ang_pid.pos = pose.theta
            self.twist.angular.z = self.ang_pid.compute()
            if abs(self.ang_pid.sp - self.ang_pid.pos) < 0.01:
                self.twist.linear.x = 0
    ...
    def on_sp_heading(self, msg:Float32):
        self.mode = Mode.HEADING
        self.pose_control_started = True
        # self.sp_heading = msg.data
        self.ang_pid.sp = msg.data
        rospy.loginfo('ON SP HEADING')
    ...
    def __init__(self) -> None:
        ...
        rospy.Subscriber('/sp_heading', Float32, self.on_sp_heading)
